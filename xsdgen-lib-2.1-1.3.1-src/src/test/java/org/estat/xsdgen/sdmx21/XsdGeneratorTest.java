...
import org.estat.xsdgen.sdmx21.XsdGenerator;
...
abstract class XsdGeneratorTest {
    ...
    /**
     * Tests the XSD generation for a SDMX 2.1 DSD.
     *
     * @param dsdFile       the DSD file
     * @param sampleFile    the sample file
     * @param options       the options
     * @param xsdOutputFile the name of the output file if XSD should be saved
     *
     * @throws Exception if fails
     */
    protected void testSdmxSample(String dsdFile, String sampleFile, XsdGeneratorOptions options,
            String xsdOutputFile) throws Exception {
        String samplesPath = "sdmx_2_1/samples/";
        String dsdPath = samplesPath + dsdFile;

        XsdGenerator generator = XsdGeneratorFactory.getInstance().newGeneratorInstance();
        Document xsd = generator.generate(parseTestFile(dsdPath), options,
                new TestURIResolver(new File(getClass().getResource(dsdPath).toURI()).getParentFile().toURI()));
        writeDocToFileIfEnabled(xsd, xsdOutputFile);
        validateMessage(xsd, samplesPath + sampleFile);
    }
}
